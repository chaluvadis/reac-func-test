            ...
            <ul className="data-list">
              {
                this.state.news.map(n => {
                  return <li className="data-list-item" key={n.id} id={n.id}>{n.id} - {n.title}</li>;
                })
              }
            </ul>
          </div>
          <div className="data-component-actions">
            <form className="form" name={this.props.mod} onSubmit={this.handleForm}>
              <input type="text" onChange={this.handleChange} value={this.state.input} className="data-input" />
              <p className="actions">
                <button className={`btn ${this.props.mod}`} onClick={this.handleForm} type="button">Publish</button>
              </p>
            </form>
          </div>
        </div>
